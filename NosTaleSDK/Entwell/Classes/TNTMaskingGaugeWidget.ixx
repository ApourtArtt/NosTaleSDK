...
#pragma pack(push, 1)
	export struct TNTMaskingGaugeWidget : public TLBSWidget
	{
		...
		struct TEWCustomPanelWidget* gaugeBackground;						// 0x028
		struct TEWCustomPanelWidget* unknownPanel2;							// 0x02C
		struct TEWCustomPanelWidget* maskPanel;								// 0x030
		struct TEWCustomPanelWidget* borderPanel;							// 0x034
		char unknown3[16];													// 0x038
		int32_t maxValue;													// 0x048
		int32_t currentValue;												// 0x04C
		int32_t bonusValue;													// 0x050
		float percentValue;													// 0x054
		struct NosTaleSDK::Entwell::Properties::Logical::Callback onUpdate;	// 0x058
